...
int main() {
    ...
    VideoCapture capture("/Users/anirudhramesh/Desktop/CSCI731_finalprojectcode/drivingVideo-2.mp4");
    if (!capture.isOpened()) {
    ...
    for (int i =0; i <5600; i++){
        ...
        if (i %2 == 0 or i%2 != 0){
            ...
            int x = frame1.rows;
            int y = frame1.cols; 
            vertices.push_back(Point(150,550));                 // Bottom-left corner
            vertices.push_back(Point(250, 0));                // Top-left corner
            vertices.push_back(Point(650, 0));                // Top-right corner
            vertices.push_back(Point(800,550));                 // Bottom-right corner
            ...
            int min_line_len = 15; // minimum number of pixels making up a line
            int max_line_gap =50; // maximum gap in pixels between connectable line segments
            vector<Vec4i> lines;
            ...
            if (addedP && addedN) {
                ...
                int t0 = (0-FitedLine[3])/FitedLine[1]; // Calculate the x-coordinate for y = 0
                ...
                // Store the corners of the frame and the homography coordinates and 
                // convert the image with the lines plotted back to the dashcam point of view. 
                Point2f corner1(0, 0);
                Point2f corner2(frame1.cols  ,0 );
                Point2f corner3(frame1.cols , frame1.rows );
                Point2f corner4(0, frame1.rows );
                vector<Point2f> lane_points = { corner4, corner1, corner2, corner3 };

                vector<Point2f> dash_cam_corners = {Point2f(-400, frame.rows), 
                                                    Point2f(350 , 455), 
                                                    Point2f(840 ,455), 
                                                    Point2f(1350, frame.rows )};

                // Perform homography 
                Mat homography_matrix = getPerspectiveTransform(lane_points, dash_cam_corners);
                Mat projected_image_x;
                warpPerspective(Blended_image, projected_image_x, homography_matrix, frame.size());
                addWeighted(frame2, 0.4, projected_image_x, 0.8, 0, frame);
                imshow("Blended Image", frame); 
                if (waitKey(30) >= 27)
                break;  
            }
        }
