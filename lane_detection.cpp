 ...
 * @brief Lane detection on roads for self-driving cars. 
...
int main(int argc, char** argv) {
	...
	//---------------GET IMAGE---------------------
	 ...
	 Mat image = imread("/Users/anirudhramesh/Desktop/opencv/PROJECT/1.jpg"); // read the image into mat type variable, image, if funning from cmd line, path is ../src/1.jpg
	 ...
	 String originalWindowName = "Original Image";
	 ...
	 String grayscaleWindowName = "Grayscaled image";

	 // Show grayscale image
	 imshow(grayscaleWindowName, imageGray);
	 ...
	 Mat smoothedIm;
	 GaussianBlur(imageGray, smoothedIm, Size(kernelSize, kernelSize), 0,0);

	 // window for smoothed image
	 String smoothedWindowName = "Smoothed image";

	 // Show smoothed image
	 imshow(smoothedWindowName, smoothedIm);
	 waitKey(0); // wait for a key press
	 ...
	 Mat edgesIm;
	 Canny(smoothedIm, edgesIm, minVal, maxVal);

	 // window for edge detection image
	 String edgeWindowName = "edge detection image";

	 // Show edge detection image
	 imshow(edgeWindowName, edgesIm);
	 waitKey(0); // wait for a key press
	 ...
	 Mat mask(image.size().height, image.size().width, CV_8UC1, Scalar(0)); // CV_8UC3 to make it a 3 channel

	 // Define the points for the mask
	 // Use Point type for x,y points
	 Point p1 = Point(0,image.size().height);
	 Point p2 = Point(455, 320);
	 Point p3 = Point(505, 320);
	 Point p4 = Point(image.size().width, image.size().height);

	 // create vector from array with points we just defined
	 Point vertices1[] = {p1,p2,p3,p4};
	 std::vector<Point> vertices (vertices1, vertices1 + sizeof(vertices1) / sizeof(Point));

	 // Create vector of vectors, add the vertices we defined above
	 // (you could add multiple other similar contours to this vector)
	 std::vector<std::vector<Point> > verticesToFill;
	 ...
	 if (!lines.empty() && lines.size() > 2) {
		 ...
		 Mat allLinesIm(image.size().height, image.size().width, CV_8UC3, Scalar(0,0,0)); // CV_8UC3 to make it a 3 channel)
	...
	y1 = int(y1 + .5);
	y2 = int(y2 + .5);
	line(laneLineImage, Point(x1, image.size().height-y1), Point(x2, image.size().height - y2),
																						Scalar(0,255,0), 3, 8 );
	...
	x2N = int(x2N + .5);
	y1N = int(y1N + .5);
	line(laneLineImage, Point(x1N, image.size().height-y1N), Point(x2N, image.size().height - y2),
																						Scalar(0,255,0), 3, 8 );
	...
	// Plot positive and negative lane lines
	 ...
	 std::vector<Point> verticesVecBlend (verticesBlend, verticesBlend + sizeof(verticesBlend) / sizeof(Point));

	 // Create vector of vectors to be used in fillPoly, add the vertices we defined above
	 std::vector<std::vector<Point> > verticesfp;
	 ...
	 // Plot lane lines
	 line(blendedIm, Point(x1, image.size().height-y1), Point(x2, image.size().height - y2),
	 																						Scalar(0,255,0), 8, 8 );
	 line(blendedIm, Point(x1N, image.size().height-y1N), Point(x2N, image.size().height - y2),
	 																						Scalar(0,255,0), 8, 8 );
